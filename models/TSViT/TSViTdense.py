...
class TSViTDown(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(self.dim),
            nn.Linear(self.dim, int(self.downsample_factor * self.patch_size)**2)
        )
    ...
    def forward(self, x):
        ...
        x = self.space_transformer(x)
        x = self.mlp_head(x.reshape(-1, self.dim))
        x = x.reshape(B, self.num_classes, self.num_patches_1d**2, int(self.downsample_factor * self.patch_size)**2).permute(0, 2, 3, 1)
